...
import { moveWidgetPlugin } from "move-widget";

export let _plugin: MoveTodoPlugin;
...
export default class MoveTodoPlugin extends Plugin {
	...
	async onload() {
		...
		// Insert our move todo button
		this.registerEditorExtension([moveWidgetPlugin]);
		// TODO: make sure that undo works as expected
	}
	...
	async editorCallback(editor: Editor) {
		...
		new Notice(
			`Moved ${todos.length} todo${todos.length === 1 ? "" : "s"} to ${
				note.basename
			}`
		);
	}
